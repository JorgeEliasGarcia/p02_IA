...
*
*  vector_t.h: Contiene la declaración de la clase vector_t, para hacer uso de matrices a partir de estos vectores
*/
...
void vector_t<T>::write(ostream& os) const {
  os << get_size() << ": [ ";
  for (int i = 0; i < get_size(); i++) os << at(i) << (i != get_size() - 1 ? "\t" : "");
  os << " ]" << endl;
}
